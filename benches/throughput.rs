...
fn bench_single(c: &mut Criterion) {
    ...
    group.bench_function("md5-many-md5-scalar", |b| {
...
fn bench_many(c: &mut Criterion) {
    ...
    for &size in &[64usize, 1024, 64 * 1024, 1024 * 1024] {
        ...
        group.bench_with_input(
            BenchmarkId::new(format!("md5-many-{}-way", lanes), size),
            &size,
            |b, _| {
                b.iter(|| {
...
criterion_group!(benches, bench_single, bench_many, bench_lane_fill);
