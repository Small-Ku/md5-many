...
The PR sentinel suite runs on both `ubuntu-24.04` x86-64 and `ubuntu-24.04-arm` AArch64. Manual `workflow_dispatch` defaults to the latest published prerelease baseline and can select the full Criterion suite; manual runs are non-blocking by default. If baseline and candidate resolve to the same SHA, the guard explicitly enters calibration mode and treats all apparent changes as runner noise. Do not turn small cross-run throughput differences into hard gates; add a paired sentinel when a scheduler/backend crossover needs protection.
