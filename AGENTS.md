...
The sentinel suite runs on both `ubuntu-24.04` x86-64 and `ubuntu-24.04-arm` AArch64 only after correctness/MSRV/quality jobs succeed. Pull requests compare the PR head against its base SHA; `master` pushes and manual CI runs compare the candidate against the latest previous reachable release tag automatically. `.github/workflows/performance.yml` is reserved for manual full-suite investigation: `workflow_dispatch` automatically selects the highest version-like release tag reachable from the candidate while excluding tags that point at the candidate itself; this naturally advances after each release, and `compare_ref` remains an optional override. Manual full runs are non-blocking by default unless `enforce` is selected. If baseline and candidate resolve to the same SHA, the guard explicitly enters calibration mode and treats all apparent changes as runner noise. Do not turn small cross-run throughput differences into hard gates; add or adjust a CI sentinel when a scheduler/backend crossover needs protection.
